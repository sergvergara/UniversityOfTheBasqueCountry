...
public class AudioOutputElm extends CircuitElm {
    ...
    void getInfo(String arr[]) {
        ...
        arr[2] = "start = " + getUnitText(dataFull
                                          ? sim.t - duration
                                          : dataStart, "s");
        arr[3] = "dur = " + getUnitText(dur, "s");
        arr[4] = "samples = " + ct + (dataFull
                                      ? ""
                                      : "/" + dataCount);
    }
...
//      int frac = (int)Math.round(Math.max(sampleStep*33000, 1));
        ...
        if (sim.timeStep != target) {
            if (okToChangeTimeStep
                    || Window.confirm(sim.LS("Adjust timestep for best audio quality and performance?"))) {
                sim.timeStep       = target;
