...
//      width = max(32, abs(yy-y));
    ...
    void draw(Graphics g) {
        ...
        for (i = 0; i != coilCount; i++) {
            ...
            drawCoil(g, ((i >= primaryCoils)
                         ? -6
                         : 6), nodeTaps[n], nodeTaps[n + 1], volts[n], volts[n + 1]);

            if (dots != null) {
                g.setColor(needsHighlight()
                           ? selectColor
                           : lightGrayColor);
        ...
        for (i = 0; i != coilCount; i++) {
            for (j = 0; j != i; j++) {
                xformMatrix[i][j] = xformMatrix[j][i] = couplingCoef
                                                        ...
                                                        * coilPolarities[i] * coilPolarities[j];
            }
        }

        CirSim.invertMatrix(xformMatrix, coilCount);

        double ts = isTrapezoidal()
                    ? sim.timeStep / 2
                    : sim.timeStep;
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) {
            EditInfo ei = new EditInfo("<a href=\"customtransformer.html\" target=\"_blank\">Description</a>",
                                       0,
                                       -1,
                                       -1);
    ...
    void setPoints() {
        ...
        int i;
        int primaryNodes = (primaryCoils == coilCount)
                           ? nodeCount
                           : coilNodes[primaryCoils];
        ...
        for (step = 0; step != 2; step++) {
            ...
            for (i = 0; i != nodeCount; i++) {
                ...
                if (step == 1) {
                    ...
                    interpPoint(point1, point2, nodePoints[i], (i < primaryNodes)
                                                               ? 0
                                                               : 1, -offset);
                    interpPoint(point1, point2, nodeTaps[i], (i < primaryNodes)
                                                             ? ce
                                                             : 1 - ce, -offset);
                }
        ...
        if (needDots) {
            ...
            for (i = 0; i != coilCount; i++) {
                ...
                dots[i] = interpPoint(nodeTaps[n], nodeTaps[n + 1], (coilPolarities[i] > 0)
                                                                    ? dotp
                                                                    : 1 - dotp, (i < primaryCoils)
                                                                                ? -7
                                                                                : 7);
            }
        } else {
