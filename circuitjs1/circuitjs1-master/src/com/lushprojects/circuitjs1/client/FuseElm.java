...
class FuseElm extends CircuitElm {
    ...
    void calculateCurrent() {
        current = (volts[0] - volts[1]) / (blown
                                           ? blownResistance
                                           : resistance);
    }

    void doStep() {
        sim.stampResistor(nodes[0],
                          nodes[1],
                          blown
                          ? blownResistance
                          : resistance);
    ...
    void draw(Graphics g) {
        ...
        g.context.transform(((double) (lead2.x - lead1.x)) / len,
    ...
    Color getTempColor(Graphics g) {
        ...
        if (temp < .3333) {
            ...
            return new Color(x + (255 - x) * c.getRed() / 255,
                             (255 - x) * c.getGreen() / 255,
                             (255 - x) * c.getBlue() / 255);
        }
