...
class PotElm extends CircuitElm implements Command, MouseWheelHandler {
    ...
    void draw(Graphics g) {
        ...
        int    i;
        int    ox = 0;
        int    hs = sim.euroResistorCheckItem.getState()
                    ? 6
                    : 8;
        double v1 = volts[0];
        double v2 = volts[1];
        ...
        if (sim.showValuesCheckItem.getState() && (resistance1 > 0) && (flags & FLAG_SHOW_VALUES) != 0) {
            ...
            // draw units
            String s1 = getShortUnitText(rev
                                         ? resistance2
                                         : resistance1, "");
            String s2 = getShortUnitText(rev
                                         ? resistance1
                                         : resistance2, "");
            ...
            // vertical?
            if (lead1.x == lead2.x) {
                g.drawString(s1, !reverseY
                                 ? arrowPoint.x + 2
                                 : arrowPoint.x - 2 - w, Math.max(arrow1.y, arrow2.y) + 5 + ya);
            } else {
                g.drawString(s1, Math.min(arrow1.x, arrow2.x) - 2 - w, !reverseX
                                                                       ? arrowPoint.y + 4 + ya
                                                                       : arrowPoint.y - 4);
            }
            ...
            if (lead1.x == lead2.x) {
                g.drawString(s2, !reverseY
                                 ? arrowPoint.x + 2
                                 : arrowPoint.x - 2 - w, Math.min(arrow1.y, arrow2.y) - 3);
            } else {
                g.drawString(s2, Math.max(arrow1.x, arrow2.x) + 2, !reverseX
                                                                   ? arrowPoint.y + 4 + ya
                                                                   : arrowPoint.y - 4);
            }
        }
