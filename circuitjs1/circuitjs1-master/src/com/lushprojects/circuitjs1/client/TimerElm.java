...
class TimerElm extends ChipElm {
    ...
    void calculateCurrent() {
        ...
        double groundVolts = hasGroundPin()
                             ? volts[N_GND]
                             : 0;

        pins[N_CTL].current = -(volts[N_CTL] - groundVolts) / 10000 - pins[N_VIN].current;
        pins[N_DIS].current = (!out)
                              ? -(volts[N_DIS] - groundVolts) / 10
                              : 0;
        pins[N_OUT].current = -(volts[N_OUT] - (out
                                                ? volts[N_VIN]
                                                : groundVolts));
    ...
    void doStep() {
        ...
        sim.stampResistor(out
                          ? nodes[N_VIN]
                          : ground, nodes[N_OUT], 1);
    }
